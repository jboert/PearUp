...
function renderBody (text) {
  ...
  // Match @word (peer names or "everyone")
...
input.addEventListener('keydown', (e) => {
  if (e.key === 'Enter' && !e.shiftKey) {
    // Don't send if mention popup is open (it handles Enter separately)
...
input.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'ArrowDown') {
    ...
    if (mentionSelectedIdx >= 0 && options[mentionSelectedIdx]) {
      e.preventDefault()
      e.stopPropagation()
      const name = options[mentionSelectedIdx].querySelector('.mention-name').textContent.slice(1) // remove @
