...
export class DaemonCore extends EventEmitter {
  ...
  async start () {
    ...
    // Join all room topics (with timeout so tray app isn't blocked)
    for (const [roomId, room] of this.rooms) {
      room.discovery = this.swarm.join(room.topic, { server: true, client: true })
      log('info', 'Flushing discovery for room', roomId)
      try {
        await Promise.race([
          room.discovery.flushed(),
          new Promise((_, reject) => setTimeout(() => reject(new Error('flush timeout')), 10000))
        ])
      } catch (err) {
        log('warn', 'Discovery flush timeout for room', roomId, '- continuing anyway')
      }
  ...
  _notifyClaude (msg) {
    ...
    const tsStr = new Date(msg.ts).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' })
    const prompt = `[PearUp]${roomContext} Message from ${msg.from} at ${tsStr}${toContext}: ${msg.body}
